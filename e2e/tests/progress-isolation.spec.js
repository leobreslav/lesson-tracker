...
test('автор свой черновик видит и помечен меткой', async ({ page, signIn }) => {
  ...
  /*
   * Метка тут одна, и она же переключатель — `VisibilityBadge`, общая с
   * витриной и со страницей заготовки.
   *
   * Меток было две. Сперва «черновик» и «веду»: вторая отвечала на вопрос
   * «какую из моих записей перепишет кнопка», и вопрос снят тем, что запись
   * называют явно. Потом «черновик» соседствовала со ссылкой
   * «Опубликовать» — то есть состояние и его переключатель стояли в двух
   * разных местах строки, а опубликованное не отвечало вовсе, молчало
   * пустым местом.
   */
  await expect(
    draft.getByRole('button', { name: 'только для меня' }),
  ).toBeVisible()
