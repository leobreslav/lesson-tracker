...
test('пока связь не записана, план со страницы урока не правится', async ({
  ...
  //
  // Записанной связи здесь нет по построению: связать можно только
  // прошедшее, а открывается будущий час. Открытый туннель проверяют тесты
  // на живом курсе ниже и питоновские.
  const slot = await upcomingSlot(api)
  await signIn(PEOPLE.ivanova)
  await openLesson(page, `${slot.date}:${slot.lesson_number}`)
  ...
  // стала бы ложью после утренней пожарной тревоги. Час впереди, поэтому
  // здесь проверяется именно эта половина правила
...
/**
 * Ближайший будущий час «Grade 6 Algebra» — тому, кто проверяет «занятие
 * ещё не проведено».
 *
 * `MONDAY:1` таким часом был до 7 сентября 2026 и в этот день перестал:
 * посеянный год зашит, а «сегодня» по нему идёт, и тест, написавший будущее
 * числом, покраснел ровно в тот день, когда число наступило. Поэтому
 * будущее спрашивается у расписания, а не пишется. Отсчёт от послезавтра,
 * а не от завтра: у сервера свой часовой пояс, и вечером по UTC «завтра»
 * может оказаться его «сегодня» — то есть уже записываемым часом.
 *
 * Живёт это до конца посеянного года: в июне 2027 будущего часа в нём не
 * останется, и тест скажет об этом словами, а не селектором.
 */
async function upcomingSlot(api) {
  const teacher = await api(PEOPLE.ivanova)
  const from = new Date()
  from.setDate(from.getDate() + 2)
  const answer = await teacher.get(
    `/api/slots/?start=${from.toISOString().slice(0, 10)}&end=2027-05-31`,
  )
  expect(answer.status, JSON.stringify(answer.body)).toBe(200)
  const slot = answer.body
    .filter((one) => one.course_name === 'Grade 6 Algebra' && !one.is_cancelled)
    .sort((a, b) => a.date.localeCompare(b.date) || a.lesson_number - b.lesson_number)[0]
  expect(slot, 'будущего часа алгебры в посеянном году не осталось').toBeTruthy()
  return slot
}
 ...
 * дни в поисках занятия теперь не нужно: неделя видна сеткой. Неделя
 * берётся из самой клетки, а не из `MONDAY`: клетку зовут и из будущего.
 */
async function openLesson(page, cell = `${MONDAY}:1`) {
  await openWeek(page, cell.split(':')[0])
...
test('часу без строки плана предлагают дописать её, а не тупик', async ({
  ...
  // четвёртый час — строк на него уже не осталось. Номер — следом за тем,
  // что выбрал себе живой курс: он у него не единица, а первый свободный у
  // Ивановой, и зашитая двойка однажды совпала с ним
  const spare = await teacher.post('/api/slots/', {
    course: course.id,
    date: slots[2].date,
    lesson_number: slots[2].lesson_number + 1,
  })
...
test('листание недель не уносит страницу в начало', async ({ page, signIn }) => {
  ...
  //
  // Мерка — «не ниже обеих недель», а не «равна прежней». Равенства тест
  // требовал, и с 1 сентября это стало неправдой без единой правки кода:
  // посеянный год начался, «текущая» неделя уехала вперёд, и соседняя
  // оказалась на 18 px выше — у неё законно другое содержимое. Схлопывание
  // до «Загрузка…» эта мерка ловит по-прежнему: такой кадр ниже обеих.
  expect(heights.min, 'страница схлопнулась на время загрузки').toBe(
    Math.min(heights.before, heights.last),
  )
  await expect(page.locator('.week-grid')).toBeVisible()
