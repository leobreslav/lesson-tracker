...
test('над таблицей одна панель управления, а под таблицей пусто', async ({
  ...
  // У этого курса свой шаблон на полке уже есть, поэтому пунктов три: взять
  // чужой, обновить свой ведомый и положить рядом копию. Раньше здесь стояло
  // `/в библиотек/`, и оно ловило сразу два последних — то есть проверяло не
  // то, что в меню есть нужный пункт, а то, что там есть хоть какой-нибудь.
  for (const name of [
    /^Открыть библиотеку/,
    /^Обновить в библиотеке/,
    /^Сохранить копию в библиотеку/,
  ]) {
    await expect(
      tools.locator('.dropdown').getByRole('button', { name }),
    ).toBeVisible()
