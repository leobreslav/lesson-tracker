...
test('над таблицей одна панель управления, а под таблицей пусто', async ({
  ...
  // Пунктов два, и оба с постоянной надписью. Было три, и два из них
  // переписывали себя по догадке «есть ли на полке мой шаблон по этому
  // предмету»: «Сохранить» превращалось в «Обновить», а рядом появлялась
  // «Сохранить копию». Проверять это списком приходилось поимённо — здесь
  // стояло `/в библиотек/`, и оно ловило любой из двух, то есть проверяло не
  // наличие нужного пункта, а наличие хоть какого-нибудь.
  for (const name of [/^Открыть библиотеку/, /^Сохранить в библиотеку/]) {
    await expect(
      tools.locator('.dropdown').getByRole('button', { name }),
    ).toBeVisible()
  }
  // и ни одного переключающегося
  await expect(
    tools.locator('.dropdown').getByRole('button', { name: /^Обновить/ }),
  ).toHaveCount(0)
