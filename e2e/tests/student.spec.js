...
test('переключатель «войти как» берёт токен нужного человека', async ({
  ...
  // суперпользователи — своей группой и первыми: возвращаются обычно
  // именно к ним, а в общем списке разработчик стоял между учителями и
  // ничем не отличался
  await expect(switcher.locator('.switch-group').first()).toHaveText(
    'Суперпользователи',
  )
  await expect(
    switcher.getByRole('menuitem', { name: 'Дмитрий Разработчиков' }),
  ).toContainText('суперпользователь')

  // подмены ещё не было — и дороги домой тоже нет: кнопка, нажатие на
  // которую ничего не меняет, хуже отсутствующей
  await expect(switcher.locator('.switch-home')).toHaveCount(0)
