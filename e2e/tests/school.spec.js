...
test('фильтры школьного расписания сужают друг друга, а не пересекаются', async ({
  ...
  // Выбор учителя назвал и год: оба курса Ивановой в одной параллели, а
  // доназначают вверх всё, что определено однозначно.
  const grade = page.getByLabel('Год обучения:')
  await expect(grade).not.toHaveValue('')

  // «все учителя» — шаг назад по цепочке, а не второе условие. Год при этом
  // остаётся: он **шире** учителя, и отменяли не его
  ...
  // а «любой год» — самый широкий уровень, и он возвращает всё
  await grade.selectOption({ label: 'любой год' })
  await expect(course.locator('option')).toHaveText([
    'все курсы',
    'Grade 6 Algebra',
    'Grade 6 Geometry',
  ...
  // В учителях — те, кто ведёт этот предмет **в этом году обучения**.
  // Курс был девятой параллели, значит год девятый; геометрию в девятом
  // ведёт только Петров, а Иванова — в шестой. Предложить её здесь значило
  // бы предложить пустую неделю: ровно то, от чего цепочка и заведена.
  await expect(teacher.locator('option')).toHaveText(['все', 'Пётр Петров'])

  // и это сужение именно годом: отпустили год — вернулись все, кто ведёт
  // геометрию хоть в какой параллели
  await grade.selectOption({ label: 'любой год' })
...
test('день школы разворачивает часы по столбцам', async ({ page, signIn, api }) => {
  ...
  /*
   * Час нужен свободный **у всех**, а не только в этом столбце.
   *
   * Клетки дня разведены по учителям (`дата:номер:столбец`), а в неделе на
   * час приходится одна ячейка (`дата:номер`) — значит проверка «в неделе
   * ровно один» верна лишь тогда, когда этот час не занят никем другим.
   * Пока брали первую попавшуюся свободную клетку, тест держался на том, на
   * какой день недели встанет анкер: в один день час пустовал у всех, в
   * другой у соседнего учителя там уже стоял урок, и в неделе находилось два.
   * Зелёный он был поэтому не всегда, а по календарю — и покраснел ровно
   * тогда, когда «сегодня» сдвинулось на сутки.
   */
  const busy = await page
    .locator('[data-lesson]')
    .evaluateAll((cells) =>
      cells.map((cell) => cell.getAttribute('data-lesson').split(':')[1]),
    )
  const spot = spots.find((one) => {
    const [, hour, key] = one.split(':')
    return ownerOf(key) && !busy.includes(hour)
  })
  expect(spot, 'свободного у всех часа в столбце учителя с курсами не нашлось').toBeTruthy()
...
test('в дне столбцы переключаются на кабинеты, и час виден там же', async ({
  ...
  // и обратно на учителей — тем же тумблером. Учителя тут не случайны: это
  // ось по умолчанию, поэтому из адреса уходит и сам параметр
  await page.getByRole('radio', { name: 'Учителя' }).click()
  await ready(page)
  await expect(page).not.toHaveURL(/by=room/)

  // оси курсов в тумблере нет вовсе: столбец с названием курса пересказывал
  // фильтры и молчал о том, где урок идёт и кто его ведёт
  await expect(page.getByRole('radio', { name: 'Курсы' })).toHaveCount(0)
})
