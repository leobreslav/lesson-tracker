...
/**
 * Открыть план с витрины — так, как это делает человек.
 *
 * Селекта в шапке «Учебного плана» больше нет: выбирают на витрине, а у
 * открытого плана на его месте стоит заголовок и дорога назад. Полтора
 * десятка тестов ходили через селект, и каждый повторял бы теперь одну и ту
 * же раскладку витрины — поэтому она названа здесь один раз.
 *
 * Щёлкает по **записи**, а не по названию внутри неё: строка целиком кнопка,
 * и попадать в имя — работа, которой человек не делает.
 *
 * Тесты, которые знают номер курса, а не название, ходят прямее — прямо по
 * адресу `/plan?course=<id>`: у плана он теперь есть, и ссылками его
 * присылают. Это не обход интерфейса, а вторая настоящая дверь.
 */
export async function pickPlan(page, name) {
  await page.locator('.showcase-item', { hasText: name }).first().click()
}

/**
 * Выбрать курс на витрине работ — карточкой, как это делает человек.
 *
 * Нужен потому, что экран работ курс за человека **не выбирает**: работы
 * заводят, правят и проверяют, а проверенное уходит ученикам — экран,
 * открывшийся на чужом курсе, выглядит ровно как открывшийся на своём.
 *
 * Живёт в оснастке, а не строкой в каждом файле, и это уже окупилось дважды.
 * Пока строка была своя у каждого, четыре файла её имели, а пять молча
 * полагались на выбор за них; а когда селект сменился витриной, правка вышла
 * одна вместо девяти. Разъезжаются такие списки не заметно, а разом.
 */
export async function pickCourse(page, name = 'Grade 6 Algebra') {
  await page.locator('.showcase-item', { hasText: name }).first().click()
}
