...
- **Google-вход не работает на NEW при проверке через `/etc/hosts`.** Это
  ...
  проходит.
- **`sudo` на OLD спрашивает пароль.** План исходит из NOPASSWD, а у старой
  машины его не было: `ssh OLD 'sudo tar …' | …` падает, не начав. Архив
  сертификатов снимается человеком в интерактивном `ssh -t` в домашний
  каталог, дальше он едет обычным `cat` и удаляется с OLD — в нём закрытые
  ключи.
- **Блок заливки, пущенный через `ssh NEW 'bash -s' < файл`, обрывается
  молча.** Первый же `docker compose exec -T` съедает остаток скрипта как
  свой stdin (см. CLAUDE.md про `-T`): схема снесена, дамп не залит, ошибки
  нет. Скрипт кладётся на NEW файлом и запускается `ssh NEW bash файл`, а у
  каждого `exec` без входных данных стоит `< /dev/null`.

## Как прошло 25 сентября 2026

Коротким путём, NEW — `159.65.114.122`. Простой от остановки OLD до
переключения DNS около часа, почти весь — ожидание публикации зоны у reg.ru:
изменение в панели видно сразу, а `ns1/ns2.reg.ru` отдают его спустя
десятки минут. Проверяется серийным номером SOA прямо на авторитетных
серверах, а не публичными резолверами. Дампы — на ноутбуке в
`~/backups/migrate-2026-09-25/`.
