...
общее, — ключи Google, и то лишь у этих трёх (клиент один,
`http://localhost` в разрешённых origins). У стенда клиент **свой**: пока он
был общим с продом, токен со стенда принимался продом — `client_id` это
аудитория `id_token`. Боевой бакет R2 не подключается ни к чему, кроме сервера:
...
Без ноутбука и без ssh — контуры подтягивают ветки сами, а ветки двигает
`ship.sh` (нужен только `gh`):

```bash
./scripts/ship.sh              # влить ветку в main => стенд, до 3 мин
