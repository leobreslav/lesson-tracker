...
`http://localhost` в разрешённых origins). Боевой бакет R2 не подключается ни к чему, кроме сервера:
...
Без ноутбука и без ssh — прод подтягивает ветку `production` сам, а ветки
двигает `ship.sh` (нужен только `gh`):

```bash
./scripts/ship.sh              # влить ветку в main => полный CI на GitHub
...
повышает названных **каждый** старт, и заведён ради базы, которую сносят и
пересевают. На проде пуст.
