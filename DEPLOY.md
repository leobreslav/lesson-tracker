...
Школу и первого администратора заводят через `/admin/` — из интерфейса это
сделать нечем, иначе любой вошедший заводил бы себе школу сам.

1. `/admin/` → **Schools → Schools → Add** → название школы.
2. **Accounts → Users** → ваш пользователь → блок **School**: выбрать школу,
   поставить галочку **School administrator**.
3. Дальше всё из интерфейса: раздел **Школа** → «Учителя» → пригласить
   коллегу по email; он войдёт через Google на этот адрес и попадёт в школу
   с той ролью, которая указана в приглашении.
