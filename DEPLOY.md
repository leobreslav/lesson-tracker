...
Его везёт `scripts/sync-env.sh`, и реализация намеренно одна — своей копии
у `push-deploy.sh` нет: две копии одного правила разъезжаются, а
...
Есть и вторая переменная, `BOOTSTRAP_SUPERUSERS` — адреса через запятую,
которые суперпользователи **всегда**, при каждом старте контейнера. Она для
базы, которую сносят и пересевают: посев заводит своего
`developer@example.com`, и первым же прогоном закрывает дверь первой
переменной. Права она только даёт (исчезнувший из списка адрес никого не
понижает) и пароля не трогает. На проде намеренно пуста.
...
### Без ноутбука: ветка `production`

Сервер стоит на ветке `production` и подтягивает её сам — `scripts/
prod-autodeploy.sh` в crontab, раз в пять минут. Пока ветку не двигали, опрос
не делает ничего, так что автовыкаткой это не является: решение выкатить
по-прежнему человеческое, просто принимать его можно не с той единственной
машины, где лежит ssh-ключ.

```bash
./scripts/ship.sh              # влить текущую ветку в main => полный CI
...
после этого двигать ветку — иначе фронт соберётся со старым
`VITE_GOOGLE_CLIENT_ID`, который вшивается в бандл.

Что и почему устроено именно так — `.claude/rules/deploy.md`, раздел «Прод
стоит на ветке `production`».
...
Стоит дерево на нужной ветке или нет, видно там же:

```bash
git rev-parse --abbrev-ref HEAD    # должно быть production
tail -20 ~/.prod-autodeploy.log    # что делал опрос
```
