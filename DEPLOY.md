...
Его везёт `push-deploy.sh` — до `deploy.sh`, потому что сборка фронта читает
оттуда `VITE_GOOGLE_CLIENT_ID`. Перед копированием он сравнивает sha256
локального и удалённого файла:
...
* спрашивает подтверждение, делает на сервере копию `.env.prod.bak.<дата>`
  (хранятся последние три), копирует файл и ставит ему права `600`.
