...
# наружу трейсбеки Django. Пароля на контуре больше нет, так что это именно
# улица, — но улица без входа: пускает стенд только адреса из
# LOGIN_ALLOWED_EMAILS, а дев-дверь со сбросом базы требует токен допущенного.
# Наружу утекает вид трейсбека, а не доступ.
#
# Пароль тут, кстати, и раньше закрывал не всё: на /api/ он стоять не мог
# (basic-auth и токен приложения делят заголовок Authorization), то есть
# трейсбеки API были видны и тогда. Разница — в странице и /admin/.
