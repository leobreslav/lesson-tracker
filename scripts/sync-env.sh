...
#   ./scripts/sync-env.sh prod --yes        # без вопроса, для скриптов
#
...
#
# Вынесен из push-deploy.sh, когда контуров было два и второй правили прямо
# на сервере — это стоило путаницы с двумя открытыми `nano` на одном файле.
#
