...
def record_state(slots: Sequence, today, window: int = RECORD_WINDOW_DAYS) -> dict:
    """
    ...
    **Долг появляется только там, где связь хоть раз ставили.** Учителю,
    ...
    главная орала бы на человека, которому эта механика не нужна. Начал —
    пропуски значат что-то; не начал — работает позиционная догадка, как
    работала всегда.

    **Требуем ограниченное время.** Ценность записи падает быстро: назавтра
    подсказка почти наверняка верна, через две недели план успел сдвинуться,
    и человек честно не помнит, что было во вторник третьим уроком.
    Настойчивость, догнавшая его через месяц, не добывает факт, а производит
    выдумку — и выдумка эта неотличима от записи. Поэтому старое из счётчика
    уходит и досчитывается позиционно, то есть ровно так, как было до всей
    затеи.

    **Дыра при этом называется вслух**: `confirmed` из `held` — доля, на
    которой посчитано всё остальное.
    """
    held = [slot for slot in slots if slot.date <= today]
    started = any(slot.lesson_id for slot in slots)
    edge = today - timedelta(days=window)

    return {
        "held": len(held),
        "confirmed": sum(1 for slot in held if slot.lesson_id),
        "unclosed": (
            sum(1 for slot in held if not slot.lesson_id and slot.date >= edge)
            if started
            else 0
        ),
    }
