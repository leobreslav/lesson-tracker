...
def readable(user):
    """
    Курсы, чей план человеку можно **прочитать**: все курсы его школы.

    Правило шире надзора, и это не послабление ради удобства, а тот же
    ответ, который уже дан расписанию: **читает вся школа, пишет ведущий или
    администратор**. Программа курса — общий артефакт школы, а не личная
    тетрадь: по ней сверяются смежники («когда у вас производная?»),
    по ней готовят замену, по ней новый учитель понимает, что было в
    прошлом году у соседней параллели.

    До этого чужой план был виден ровно двум людям — назначенному методисту
    и администратору школы, — а всем остальным его показывала только
    библиотека, то есть **снимок**, который кто-то догадался положить на
    полку. Живой план соседа при этом не был закрыт по какой-то причине: он
    просто ни разу не был открыт.

    Права это не даёт никакого, кроме чтения: правит план его автор, решает
    по запросу методист (`supervised` ниже), а чужая школа по-прежнему
    неотличима от несуществующей.
    """
    return Course.objects.filter(school_id=user.school_id).select_related(
        "year", "subject"
    )
