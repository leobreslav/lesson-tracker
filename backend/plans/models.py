...
class PlanNode(LessonContent):
    ...
    **План принадлежит курсу, а не учителю** — в отличие от расписания.
    ...
    #: Строка, с которой эту скопировали, — **числом, а не связью**.
    #:
    #: Тот же приём, что у `PlanBaselineRow.node_id`, и по той же причине:
    #: исходную строку удалят, и именно это удаление память о происхождении
    #: должна пережить. Связь тут ещё и обещала бы не то — что правки бегут
    #: по ней, — а копия остаётся копией: взяв план с полки, вы получаете
    #: свой, и позднейшие правки шаблона до него не доходят.
    #:
    #: Нужна она ровно для одного: **сравнить** план с тем, из чего он вышел.
    #: Сравнение в проекте точное — оно ищет строки по устойчивым номерам, а
    #: не гадает по названиям, как текстовые diff'ы, — и копия эти номера
    #: рвёт. Без записи «откуда я» обновление шаблона с курса показывало бы
    #: «удалено 47, добавлено 48» ровно в самом частом случае: в сентябре
    #: «9Б Алгебра» — уже другой курс с другими строками.
    origin_id = models.PositiveIntegerField(
        "copied from this row", null=True, blank=True
    )
