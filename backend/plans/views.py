...
class PlanNodeViewSet(CourseScopedViewSet):
    ...
    def readable_templates(self):
        """
        Шаблоны, которые можно **прочитать**: свои и выложенные коллегами.

        Читать и править на полке разошлись, и это то же различие, что у
        курса: живой план курса читает вся школа, а правит ведущий. Полка
        для того и заведена, чтобы программой пользовались, — а прочитать
        выложенное было нельзя вовсе: страница плана открывала запись
        только автору, и чужая ссылка кончалась 404 на пустом экране.

        Шире это не делает ничего: `visible_templates` — тот же список, по
        которому полка и показывается, и чужие черновики в него не входят.
        То есть читать можно ровно то, что и так видно в окне библиотеки; до
        сих пор это было видно списком, но не открывалось.
        """
        from library.serializers import visible_templates

        return visible_templates(self.request.user)
    ...
    def requested_owner(self, *, write=False):
        ...
        **`write` наконец что-то значит.** Флаг стоял здесь с самого начала
        и не делал ничего: списки чтения и правки совпадали, и обе стороны
        отвечали «своё». Разошлись они у шаблона — выложенную на полку
        запись читает вся школа, а правит автор, — и место для этого
        различия уже было названо вызывающими: отмена, откат и пакетное
        удаление просят `write=True`, дерево и не просит.

        У курса различия по-прежнему нет, и это не забывчивость: чужой
        курсовой план читают не здесь, а `PlanReviewViewSet` — вместе с
        состоянием утверждения и правом решать. Второй дорогой к тому же
        дереву они бы разъехались.

        Чужое — 404, как и было, и по обоим владельцам сразу: про чужой
        черновик на полке знать незачем вовсе, а `visible_templates` их и не
        показывает.
        """
        ...
        templates = self.my_templates() if write else self.readable_templates()
        return of_template(get_object_or_404(templates, pk=raw))
    ...
    def plan_history(self, request):
        ...
        **Цели обеих кнопок считает сервер**, а не клиент по `steps[0]`.
        ...
        сервер не сдержит.
        """
        # `write=True` у списка, который ничего не пишет, — и это не описка.
        # Журнал отмены отвечает на вопрос «что я могу вернуть», а вернуть
        # может только тот, кто правит: читателю он предлагал бы кнопки,
        # которых у него нет. Поведение от флага не изменилось ни на шаг —
        # до появления чтения оба списка были одним и тем же; изменилось то,
        # что теперь это сказано, а не совпадает случайно.
        owner = self.requested_owner(write=True)
    ...
    def undo(self, request):
        ...
        **Ход записывает в журнал ровно один раз**, в самом начале, — тем
        ...
        Восстановление проверяется дважды, и обе проверки про записи о
        проведённых часах. Первая — внутри самого `restore`: строку, по
        которой провели урок, отмена не удаляет
        (`plan_undo_would_lose_record`), потому что удаление уносило бы
        запись молча. Вторая — здесь, пост-условием: после отката очередь
        записей всё ещё идёт без дыр. Отказ приходит обычным кодом, а не
        пятисоткой, и отменяет всё восстановление целиком.

        Абзац выше долго описывал не этот код: проверок не было ни одной, а
        текст утверждал, что есть. Так и держалась дыра — читающий верил
        докстрингу и дальше не смотрел.
        """
        owner = self.requested_owner(write=True)
