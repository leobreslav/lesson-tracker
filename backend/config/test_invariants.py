...
class NoTestSpendsMoneyTests(SimpleTestCase):
    ...
    гоняются они на каждом шаге работы и в CI на каждый пуш.
...
class EveryDoorAsksWhoIsComingTests(SimpleTestCase):
    ...
    которые его выдают, в проекте три, и второе легко упустить из виду —
    `/api/dev/login/` заведена ради «Войти как» в разработке, живёт за флагом
    и выглядит служебной. Ровно поэтому она и опасна: она выдаёт токен **кому
    угодно по адресу**, без Google и без пароля. Третье — вход по коду из
    письма (`accounts/codes.py`): токен по адресу и коду, для тех, у кого
    нет Google-аккаунта.
    ...
    DOORS = {
        "accounts/dev_door.py": "дверь разработки, «Войти как»: токен по адресу",
        "accounts/codes.py": "вход по коду из письма: токен по адресу и коду",
    }
...
class EveryRelationToAPlanRowSurvivesUndoOrRefusesItTests(SimpleTestCase):
    """
    Связь со строкой плана обязана сказать, что с ней делает отмена.

    Отмена восстанавливает строку **с прежним номером** — и делает это
    намеренно: за строкой стоят вложения, эталон, разметка задачника и запись
    о проведённом часе, а новая строка с новым номером для них всех не она.
    Но восстанавливает `plans.history.restore` только две вещи: поля самой
    строки и вложения. Всё остальное, что на строку ссылается, переживает
    снимок не потому, что об этом кто-то позаботился, а потому, что повезло.

    Дважды не повезло. `bank.Introduction` — `CASCADE`: удалили строку,
    пометка «этот урок вводит это понятие» ушла, отмена вернула строку, а
    пометку возвращать неоткуда, её больше нет нигде. `schedule.Slot.lesson`
    — `SET_NULL`: отмена молча развязывала связь, и час, который провели и
    записали, оставался без записи. Ни то, ни другое не было решением — оба
    случая просто никем не рассматривались.

    Отсюда сторож. Он не проверяет, что связь устроена правильно: правильных
    ответов три, и выбор между ними — дело автора связи. Он требует, чтобы
    ответ **был дан**, потому что молчание здесь выглядит в точности как
    согласие и обнаруживается через полгода потерянными данными.

    Тот же приём стоит у расписания, и там он выведен из той же беды:
    «на слоте висит чужая работа, и снимок обязан её пережить». План этот
    вывод применил к вложениям и остановился на них.
    """

    #: снимок носит связь и возвращает её при восстановлении
    CARRIED = "carried"
    #: отмена отказывается, если восстановление уносило бы эту связь
    REFUSED = "refused"
    #: связь к отмене не относится вовсе, и это тоже решение
    ASIDE = "aside"

    #: связь → что с ней делает отмена и почему именно это.
    #:
    #: Строка нужна не для отчётности: она и есть решение, которое иначе
    #: принимают молча. Три варианта не равноценны — «носим» стоит выбирать
    #: для того, что человек написал руками, «отказываемся» для того, что
    #: восстановлению неподвластно, «мимо» только тогда, когда связь
    #: восстанавливается сама.
    RELATIONS = {
        "plans.PlanNode.parent": (
            CARRIED,
            "дерево: снимок держит parent_node_id у каждой строки",
        ),
        "files.Attachment.plan_row": (
            CARRIED,
            "вложения: PlanSnapshotFile держит и ссылку на объект в бакете, "
            "чтобы тот дожил до отката",
        ),
        "bank.Introduction.node": (
            CARRIED,
            "разметка задачника: PlanSnapshotIntroduction возвращает пометку "
            "«этот урок вводит понятие» тому уроку, у которого она была",
        ),
        "schedule.Slot.lesson": (
            REFUSED,
            "запись о проведённом часе: восстановить её нельзя — час уже был, "
            "и отмена отказывается кодом plan_undo_would_lose_record",
        ),
    }

    def relations(self) -> dict:
        """Кто ссылается на строку плана — по самой модели, а не по памяти."""
        from plans.models import PlanNode

        return {
            f"{rel.related_model._meta.label}.{rel.field.name}": rel
            for rel in PlanNode._meta.related_objects
        }

    def test_every_relation_says_what_undo_does_to_it(self):
        found = self.relations()

        unknown = sorted(set(found) - set(self.RELATIONS))
        self.assertEqual(
            unknown,
            [],
            "на строку плана ссылается что-то новое, и про отмену это молчит. "
            "Решите: снимок её носит (как вложения), отмена отказывается (как "
            "с записью о часе) или она к отмене не относится, — и допишите "
            "сюда с этим ответом. Связь без ответа — это данные, которые "
            "отмена унесёт молча",
        )

        gone = sorted(set(self.RELATIONS) - set(found))
        self.assertEqual(
            gone,
            [],
            "связи больше нет, а решение про неё осталось: уберите строку, "
            "иначе следующий будет искать поле, которого нет",
        )

    def test_the_journal_knows_the_relations_it_claims_to_handle(self):
        """
        Заявленное «носим» и «отказываемся» подтверждается кодом журнала.

        Проверка грубая — журнал обязан хотя бы **поминать** модель, — и
        нарочно такая: тонкую здесь не написать, а грубая ловит ровно тот
        случай, который и был. `bank.Introduction` числился бы «носимым» с
        подробным объяснением, а в `plans/history.py` не было бы про него ни
        слова; так строка в таблице и стала бы заменой самой связи.
        """
        source = (
            Path(settings.BASE_DIR) / "plans" / "history.py"
        ).read_text(encoding="utf-8")

        silent = sorted(
            name
            for name, (verdict, _) in self.RELATIONS.items()
            if verdict in (self.CARRIED, self.REFUSED)
            and name.split(".")[1] not in source
        )
        self.assertEqual(
            silent,
            [],
            "про связь сказано «носим» или «отказываемся», а журнал о ней не "
            "знает вовсе: решение записано в таблицу, но не в код",
        )

    def test_every_decision_explains_itself(self):
        mute = sorted(
            name
            for name, (verdict, why) in self.RELATIONS.items()
            if verdict not in (self.CARRIED, self.REFUSED, self.ASIDE) or not why
        )
        self.assertEqual(
            mute,
            [],
