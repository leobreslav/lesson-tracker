...
**И дверь для разработки здесь же закрывается.** В `.env` разработки
`DEV_LOGIN=true` — иначе не войти четырнадцатью учениками без четырнадцати
гугл-аккаунтов. Но сторож двери проверяет ровно обратное: что при
выключенном флаге маршрутов нет вовсе. Если бы он читал окружение, он
проверял бы не код, а `.env` того, кто запустил тесты. Поэтому прогон всегда
идёт с закрытой дверью; разработке это не мешает — там работает runserver, а
не тест-раннер.
...
class Runner(DiscoverRunner):
    def setup_test_environment(self, **kwargs):
        ...
        self._door = override_settings(DEV_LOGIN=False)
        self._door.enable()
        # Список допущенных — по той же причине, что и дверь выше. У машины
