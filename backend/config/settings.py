...
REST_FRAMEWORK = {
    ...
    # Частота только у входа по коду: это единственное место, куда стучат
    # без токена и которое отправляет письма. Считается по адресу запроса,
    # в памяти процесса — грубо, но заваливать чужую почту этого хватает
    "DEFAULT_THROTTLE_RATES": {"login_code": "30/hour"},
}
...
# --- почта -------------------------------------------------------------------
#
# Два дела, и они разной важности. Пересылка обращений суперпользователю
# (`feedback`) — уведомление: обращения лежат в базе, письмо лишь дублирует.
# Вход по коду из письма (`accounts/codes.py`) — сама **дверь**: без письма
# человек без Google-аккаунта не войдёт вовсе.
#
# Без `EMAIL_HOST` письма печатаются в лог контейнера, и для обращений это
# **рабочее состояние**, а не заглушка. Для входа по коду — только в
# разработке (`DEBUG`), где код из лога читается; боевой контур без почты
# отказывает во входе по коду прямо (`email_login_unavailable`), а не
# печатает коды в лог, которого никто не читает. Молчащий SMTP на localhost
# был бы хуже обоих: он глотает письма, ничего не сообщая.
