...
def configured() -> bool:
    """
    Есть ли чем звать модель. Тот же вопрос, что у распознавателей.

    Спрашивают его там, где читателя **выбирают**: контур без ключа не должен
    предлагать модель человеку, а узнавать об этом отказом на первой странице
    пачки — плохой способ выяснять состав читателей.

    Это не то же, что «достаёт ли до модели» (`reach.py`): ключа может не быть
    вовсе, а может быть настоящий ключ и закрытая сеть. Чинятся эти два
    состояния по-разному, поэтому и спрашиваются порознь.
    """
...
def _client():
    ...
    # Ретраи на 429/5xx берёт на себя SDK: наш вызов и так один на страницу.
    #
    # **Но ждать соединения долго нельзя, и это стоило пятисотой на живой
    # пачке.** На контуре, откуда Anthropic не отвечает, пакеты не отбиваются,
    # а пропадают: соединение не падает, а висит. Умолчания SDK (нет предела
    # на connect, три повтора) превращали это в минуту ожидания внутри
    # запроса, а воркер gunicorn убивают по таймауту — и страница отвечала
    # `SystemExit`, то есть пятисотой без единого слова о причине.
    #
    # Пять секунд на соединение при трёх попытках дают пятнадцать секунд
    # худшего случая — после них остаётся время позвать распознавателя, ради
    # которого весь запасной путь и заведён. Повторы при этом не теряются:
    # там, где сервер **ответил** (429, 5xx), они стоят копейки и работают
    # как прежде.
...
def read_header(
    ...
    **Чужого чтения сюда не приходит вовсе, и параметра под него нет.** Был:
    арбитр показывал модели обе разошедшиеся версии и просил рассудить.
    Работал он на догадке о странице, а знание о читателях отвечает точнее —
    имя лучше читает модель, клетки распознаватель, — и отвечает бесплатно
    (`merge.py`). Отсутствие параметра тут не небрежность, а сторож: пока его
    нет, подсказку неоткуда взять.
    """
    hint = ""
    if candidates:
