...
class Kind(models.TextChoices):
    """
    Учитель, ученик или родитель. Виды пользователя, а не роли внутри одного.
    ...
    **Родитель — третий вид, а не разновидность ученика**, и это решение, а не
    оформление. Видит он то же, что его ребёнок, — и на этом сходство
    кончается:

    * у него **несколько детей**, значит у каждого его экрана есть вопрос «чей»,
      которого у ученика нет и быть не может;
    * у него **свой разговор с учителем**, которого ученик не видит;
    * оповещения у них расходятся в обе стороны: «ответ проверен» — ученику,
      «работа не сдана к сроку» — родителю, и складывать их в один вид значило
      бы показывать каждому чужое.

    Мама и папа — **две независимые учётки** на одного ребёнка. Общий вход
    выглядел бы экономией, а стоил бы правды: кто прочитал сообщение, кто
    написал учителю, кому ушло оповещение — всё это вопросы к человеку, и
    ответить на них про «родителя вообще» нельзя.

    Вид проставляется один раз, при приёме приглашения, и не меняется:
    один адрес — один вид. Две учётки разных видов на одной почте держать
    нельзя, и это проверяется при приглашении, а не при входе.
    """
...
class User(AbstractUser):
    ...
    def is_parent(self) -> bool:
        return self.kind == Kind.PARENT

    @property
    def is_teacher(self) -> bool:
        """
        Учитель — это **вид**, а не «все остальные».

        Пока видов было два, «не ученик» и «учитель» совпадали, и половина
        проверок пользовалась вторым через первое. С появлением родителя
        совпадение кончилось: родитель не ученик, а значит по старому правилу
        прошёл бы в учительские разделы целиком. Поэтому вид спрашивается
        прямо, и добавление четвёртого вида ничего не откроет само.
        """
        return self.kind == Kind.TEACHER

    @property
    def is_family(self) -> bool:
        """Ученик или его родитель: тот, кому показывают ученическое."""
        return self.kind in (Kind.STUDENT, Kind.PARENT)
