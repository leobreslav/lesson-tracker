...
class UserSerializer(serializers.ModelSerializer):
    ...
    methodist_courses = serializers.SerializerMethodField()
    ...
    def get_methodist_courses(self, obj) -> list:
        """
        Планы каких курсов этот человек утверждает.

        Отсюда интерфейс узнаёт, показывать ли ему раздел «На утверждение»:
        роль не иерархическая, и у большинства список пуст.
        """
        return [
            {"id": row.course_id, "name": row.course.name}
            for row in obj.methodist_of.select_related("course")
        ]
