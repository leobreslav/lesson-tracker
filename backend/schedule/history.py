...
class SlotSnapshot(models.Model):
    ...
    #: Что именно отменили — заполнено только у снимков самой отмены.
    #:
    #: Нужно оно кнопке. Отмена в расписании намеренно односкоростная, и
    #: второе нажатие возвращает отменённое, — но называлась она при этом
    #: по-прежнему «Отменить», а `action` у такого снимка «undo», то есть
    #: надпись выходила «Отменить: отмену». Чтобы сказать «Вернуть: перенос
    #: занятия», надо помнить, чем был перенос; вывести это из соседнего
    #: снимка по времени можно, но соседство — догадка, а здесь запись.
    undone_action = models.CharField(
        "action being undone", max_length=32, blank=True
    )
