...
EXCUSED = {
    ...
    "SlotViewSet.slot_redo": (
        "возврат отменённого: состояние уже записано, снимать нечего. "
        "Наоборот — он снимок отмены **убирает**, иначе переключение "
        "туда-обратно за десяток нажатий вытеснило бы из журнала всю "
        "настоящую историю"
    ),
}
