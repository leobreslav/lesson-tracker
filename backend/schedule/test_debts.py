...
Записывать «что прошли» стоит настойчиво, но недолго. Ценность записи
падает быстро: назавтра подсказка почти наверняка верна, через две недели
план успел сдвинуться, и человек честно не помнит, что было во вторник
третьим уроком. Настойчивость, догнавшая его через месяц, не добывает факт,
а производит выдумку — и выдумка эта неотличима от записи.

Отсюда три правила, и каждое проверяется здесь: долг появляется только у
того, кто начал записывать; он живёт две недели; закрывается пачкой, но с
подставленными темами на экране.
"""
