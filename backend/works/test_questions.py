...
У бумажной работы задача — это критерий шкалы: отдельных `Task` у неё нет по
определению. Проверяется тут одно решение и его последствия: **расхождение не
...
class QuestionTests(SchoolTestMixin, APITestCase):
    ...
    def test_the_statements_land_on_the_criteria_by_number(self):
        ...
        texts = list(self.work.criteria.values_list("question", flat=True))
    ...
    def test_questions_beyond_the_scale_are_named_not_added(self):
        ...
        self.assertEqual(self.work.criteria.count(), 3)
    ...
    def test_a_different_top_mark_is_reported_not_applied(self):
        ...
        self.assertEqual(self.work.criteria.all()[1].maximum, 3)
    ...
    def test_reading_again_overwrites_the_old_text(self):
        ...
        self.assertEqual(self.work.criteria.first().question, "новое")
    ...
    def test_a_sheet_without_questions_changes_nothing(self):
        ...
        self.assertEqual(self.work.criteria.first().question, "раз")
