...
def files_of(work, *, staff: bool = True) -> list:
    """
    ...
    Расходятся стороны ровно в одном, и по существу: `staff_only`. Учитель
    прикладывает к работе и то, что классу показывают (условия, бланк), и то,
    что не показывают (ответы, разбор, отсканированная пачка), — и видит он
    всё своё, а класс только открытое ему. Признак живёт на вложении, потому
    что решается он **на файл**, а не на работу.

    Поэтому у стороны есть имя (`staff=False` — глазами класса), а не
    отдельная функция: пока вопрос один и тот же, ответов должно быть два, но
    расчёт один. Забытый флаг тут отдаёт классу ответы к контрольной, и в
    тестах он стережётся отдельно.
    ...
    Именно «одна функция на обе стороны» и делает это место опасным: забудь
    тут условие, и спрятанное уехало бы классу молча, при живом `staff_only`
    во всех остальных дверях.
    """
...
class ScanReadSerializer(serializers.Serializer):
    ...
    # Та же шапка, но как на бумаге: без плиток и подписей. По ней читают имя.
    #
    # Разница измерена живой пачкой: распознаватель на собранном листе
    # склеивает строку имени с первым рядом плиток — «Миронова» приезжала как
    # «Леилонова», «Тимур Кузнецов» как «иму Кузнецов», — а на полоске те же
    # страницы читаются верно. Клетки наоборот: на голой полоске Mathpix не
    # видит ни одной цифры.
    #
    # Необязательна: клиент мог отстать на выкатку, и тогда всё читается по
    # собранному листу, как читалось раньше.
    plain = serializers.FileField(required=False, allow_null=True)
