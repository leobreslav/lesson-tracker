...
**Этот путь исходника не сохраняет.** Разметку руками делают один раз и по
глазам, возвращаться к той же книге вторым заходом тут не за чем, а лежала бы
она одним файлом со всеми работами класса. У разбора пачки по именам это
решение отменено — там пачка остаётся у работы под `staff_only`
(`services.attach_batch`), потому что к ней возвращаются: разбор идёт двумя
заходами, а страницы рисует браузер из файла.
"""
