...
# Состояния работы. Их четыре, и четвёртое появилось позже остальных.
#
# Здесь стояло «четвёртого нет и не должно быть», и стояло по доводу:
# «ненаступившее окно прячет работу целиком, черновика нет — вместо него
# работает именно это». Довод верен ровно наполовину. Спрятать работу окно
# действительно умеет, но **само же и показывает** её, когда время придёт, —
# то есть учитель, заведя работу и не дописав задачи, узнаёт о выдаче не в тот
# момент, когда решил выдать, а в тот, который выставился по умолчанию.
#
# Поэтому выдача стала отдельным ответом: `is_released`. Окно по-прежнему
# отвечает «когда решают», а выдача — «показывать ли вообще».
DRAFT, PLANNED, OPEN, CLOSED = "draft", "planned", "open", "closed"
...
class Work(models.Model):
    ...
    **Бумажная работа — та же работа, у которой не задействованы отправки**,
    ...
    is_released = models.BooleanField(
        ...
        default=False,
        help_text=(
            "Выдана ли работа классу. Пока нет — ученику её не существует "
            "независимо от окна времени: заводят работу пустой и дописывают "
            "задачи, а окно к этому моменту уже проставлено по умолчанию."
        ),
    )
    ...
    description = models.TextField(
        ...
        blank=True,
        help_text=(
            "Текст задания: то, что ученик видит над задачами, и то же самое "
            "видит над ними учитель. Задачи при этом необязательны — задание "
            "целиком бывает написано здесь, а решения приходят снимками."
        ),
    )
    ...
    def state(self, now=None) -> str:
        """`draft` → `planned` → `open` → `closed`.

        Первое состояние вне времени: пока работу не выдали, окно ничего не
        значит — ученику её не существует, сколько бы ни было на часах.
        """
...
class Task(models.Model):
    ...
    **Условия может не быть вовсе.** Пустая ячейка — это ячейка без условия, а
    ...
    # Как этот вопрос назван **в этой работе**: «1а», «1б», «324 из Галицкого».
    #
    # Позиция и имя — разные вещи, и путать их нельзя. Позиция говорит, в каком
    # порядке вопросы идут и какая клетка бланка чья; имя говорит, как учитель
    # его называет вслух и на доске. Пока имени не было, вторая роль
    # исполнялась первой — вопросы звались «1, 2, 3…» по порядку, — и работа с
    # пунктами «1а, 1б, 1в» становилась работой из трёх задач с чужими
    # номерами: то, что на бумаге один вопрос с тремя пунктами, выразить было
    # нечем.
    #
    # Пустое имя — рабочее состояние, а не пропуск: тогда вопрос зовётся своим
    # номером по порядку, и у большинства работ так и остаётся. Поэтому имя
    # хранится пустым, а не заполняется числом при создании: заполненное число
    # пришлось бы перебивать при каждом переносе вопроса вверх или вниз, и
    # первый же перенос сделал бы имена враньём.
    ...
    def name(self) -> str:
        """
        Как вопрос зовётся на экране: своё имя, а иначе номер по порядку.

        Одно место на все показы — таблица проверки, список задач, сводка,
        разбор скана. Считать это по позиции на каждом экране было бы четырьмя
        копиями одного правила, и разошлись бы они молча: имя появилось бы там,
        где его дописали, а в соседней таблице остался бы номер.
        """
        return self.label or str(self.position + 1)
...
class StudentWork(models.Model):
    ...
    **задачи**, — а у работы на бумаге отправок нет вовсе, зато есть скан и
    ...
    # Итог за работу, поставленный **учителем**.
    #
    # Система выводит отметку из набранного по своим полосам
    # (`services.grade_for`), и это хороший умолчательный ответ — но только
    # умолчательный. Что бы ни было заложено в пороги, последнее слово за
    # человеком: работа, где всё решено, но списано; работа на границе, где
    # видно, что ученик понял; работа, за которую учитель отвечает перед
    # родителем своим именем. Правило простое и одно: **мнение учителя
    # сильнее**.
    #
    # Хранится буквой, а не числом, и это не небрежность: отметкой бывает
    # «4», «7», «зачёт» и «B» — общего числа у них нет, а полоса системы и
    # так называется словом (`GradeBand.label`).
    #
    # Пусто — законное состояние и самое частое: считает система. Ноль тут
    # не годился бы даже как «ничего не поставлено» — «0» бывает отметкой.
    grade = models.CharField(
        "final grade set by the teacher",
        max_length=40,
        blank=True,
        help_text=(
            "Итог, поставленный руками. Пусто — считает система по своим "
            "полосам; заполнено — показывается это, а не выведенное."
        ),
    )
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
...
class ScanPage(models.Model):
    ...
    # первым. Одна модель ошибается молча — «Denis» становится «Misha», и
    # страница уходит не тому уверенно и без единой пометки; поймать это можно
    # только чужим свидетельством. Словарём, а не полями: это диагностика для
    # человека, у неё нет ни запросов по ней, ни связей. Пустой — второго
    # читателя не было вовсе, и это законное состояние.
    second = models.JSONField("what the second reader saw", default=dict, blank=True)
...
class Thread(models.Model):
    ...
    **Предмет треда — пара «вопрос работы и ученик».** Тогда он один и тот же и
    на экране задачи, и в переписке: мессенджер (`talks`) — просто другой способ
    читать те же треды, сгруппированные по собеседнику, а не вторая переписка о
    том же. Обещание это записано было заранее, и оно сбылось: тред никуда не
    переезжал, лента забирает его как есть.
...
class Message(models.Model):
    ...
    # экономию таблиц: **чат читает сообщения, сгруппированные по
    # собеседнику**, и две таблицы сошлись бы в этом месте объединением, а
    # разошлись бы в первой же правке — у одной появилась бы отметка о
    # прочтении, у другой нет. Ровно так и случилось с семейной перепиской,
    # державшей свою: непрочитанное у родителя не считалось вовсе.
    #
    # Заметку в ленту мессенджер при этом **не** берёт, и это отступление от
    # сказанного здесь раньше: она приколота к точке на картинке, и «здесь
    # потерян минус» без «здесь» — не сообщение, а загадка. Читают её там, где
    # видно место, — в просмотрщике (см. `.claude/rules/talks.md`).
