...
def grade(
    ...
    *,
    ...
    with transaction.atomic():
        ...
        #
        # Правка итога дописывается в журнал (`MarkChange` без обеих осей) —
        # ровно как правка балла, и по той же причине. Раньше эта ветка
        # записывала поле и выходила из функции раньше журнала: пока итог
        # ставили изредка из окна проверки, отсутствие истории не замечали. С
        # журналом курса, где оценку ставят прямо в клетку, руками
        # поставленный итог стал самой частой оценкой в системе — и остался
        # бы единственной, у которой нельзя спросить «а что тут было раньше».
        if final is not None:
            wanted = final.strip()[:40]
            # только **изменение**: сохранение того же значения — не событие,
            # а журнал, полный одинаковых строк, ничем не отвечает на вопрос,
            # ради которого заведён
            if wanted != row.grade:
                row.grade = wanted
                row.save(update_fields=["grade", "updated_at"])
                MarkChange.objects.create(
                    student_work=row, label=wanted, changed_by=by
                )
...
def scan_state(work) -> dict:
    ...
    for number, packet in enumerate(packets):
        ...
        # раздают **перед** работой, значит он принадлежит тому, чьи решения
        # идут следом. А на экран хозяин брался только со страниц решений, и
        # лист условий показывался ничьим — при том, что в PDF ученика он
    ...
    return {
        ...
        # Можно ли позвать второго свидетеля — Mathpix — и если нет, почему.
        # Спрашивает это галочка на шаге выбора файла, и спрашивает не из
        # любопытства: пропавшая галочка себя не объясняет, а заглушённая
        # объясняет. Контур без ключей Mathpix раньше выглядел как контур, где
        # Mathpix не бывает вовсе.
        ...
        # Кем этот контур умеет читать имя, в порядке предпочтения, и **все**
        # трое — недоступный тоже, со своей причиной. Человек выбирает между
        # ними на шаге выбора файла, и выбор этот честнее показывать целиком:
        # «Mathpix тут не настроен» — ответ, а исчезнувшая строка — загадка.
        "readers": name_readers(),
    }


def second_reader() -> dict:
    """
    Можно ли позвать второго свидетеля, и если нет — почему (`vision/services.py`).

    Словарь, а не флаг: «ключей не задали» и «не достучались» чинятся в разных
    местах, и галочка, заглушённая без объяснения, отправляет человека искать
    беду не там.
    """
    from vision import services as vision_services

    return vision_services.readers_state((vision_services.MATHPIX,))[0]
...
def name_readers() -> list:
    """Кем этот контур умеет читать имя, все со своим состоянием."""
    from vision import services as vision_services

    return vision_services.readers_state(vision_services.NAME_READERS)
