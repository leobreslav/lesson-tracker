...
def scan_state(work) -> dict:
    ...
    return {
        ...
        # Кем контур умеет читать клетки — **все** такие читатели, каждый со
        # своим состоянием и причиной отказа. Спрашивает это выбор на шаге
        # выбора файла, и спрашивает не из любопытства: пропавшая строка себя
        # не объясняет, а заглушённая объясняет. Контур без ключей Mathpix
        # раньше выглядел как контур, где Mathpix не бывает вовсе.
...
def cells_readers() -> list:
    ...
    Состояния, а не имена, и не флаг «есть ли Mathpix»: экрану надо знать не
    только, кого предложить, но и **почему** остальных предложить нельзя.
    Тонкость в том, что Mathpix читает одним способом, а Yandex двумя —
    поэтому «имя читает он же» отменяет первого и не отменяет второго; это
    считает уже экран, потому что зависит от его же выбора читателя имён.
    """
    from vision import services as vision_services

    return vision_services.readers_state(vision_services.CELLS_READERS)
