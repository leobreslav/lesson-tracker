...
#
# Список написан один раз намеренно: владельцев было три, стало шесть, потом
# пять, и перечисление их по месту — в ограничении, в `clean`, в
# сериализаторе, во вьюхе — означало бы пять списков, расходящихся молча.
# Забытый в ограничении владелец не запрещает ничего, забытый в `clean` —
# молчит, забытый в сериализаторе — не даёт приложить вовсе.
#
# **Шестым была строка шаблона, и она ушла**, а материалы полки остались: план
# на полке стал обычным планом, и строка у него та же самая, `plan_row`. Пока
# хранение было двух видов, двух владельцев требовала схема; теперь второй
# означал бы два ответа на вопрос «к чему это приложено» для одной и той же
# вещи — и первый же забытый ответ прятал бы материалы там, где их ждут.
#
...
class Attachment(models.Model):
    ...
    Пятый — та же полка, но школьная: то, что администратор кладёт всем
    сразу. От четвёртого он отличается обоими краями — читает его вся школа,
    а пишет один администратор, — и потому это владелец, а не признак
    «общее» у личной вещи: у признака остался бы хозяин, а с его уходом
    `CASCADE` унёс бы регламент у всех.

    Работа ученика отличается от первых двух правом на чтение: план и
    ...
    **ученика** о ней, а различает их `added_by`, а не отдельная таблица:
    ...
    #
    # Признак, а не ещё один владелец, и разница здесь существенная. Владелец
    ...
    # два места для учительских файлов, а `OWNER_FIELDS` — на имя длиннее в
    # каждом из четырёх мест, которые его читают. (Числа тут намеренно нет:
    # владельцев с тех пор прибавилось дважды, и счёт в прозе устарел бы
    # третий раз.)
    #
