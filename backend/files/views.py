...
class AttachmentViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        # Скрытое от класса (`staff_only`) отсюда **не** убирается, и это
        # переменилось вместе с тем, чем признак стал. Заведён он был на одну
        # вещь — отсканированную пачку, которую в списке материалов видеть
        # незачем, — а теперь это настройка любого вложения работы: учитель
        # прикладывает и условия классу, и ответы себе. Прятать от него же
        # то, что он сам спрятал от класса, значит не дать ни убрать это, ни
        # передумать. Ученику и семье оно по-прежнему недоступно, и решает
        # это `readable_attachments`, а не фильтр здесь.
    ...
    def create(self, request, *args, **kwargs):
        ...
        attachment = Attachment.objects.create(
            **owner,
            ...
            stored_file=stored,
            inline=inline,
            # видно ли это классу. Спрошено при загрузке — см. сериализатор:
            # видимость, выставляемая после, оставляет окно, в которое класс
            # успевает посмотреть
...
class ContentImageView(APIView):
    ...
    **Учительской эта дверь быть перестала, и звалась она поэтому
    ...
    ссылке на файл. Ученику своими оказываются ровно три: работы, которые ему
    открыты, условия их задач и его собственная тетрадь; ни урока, ни полки
    он не читает.
    """
