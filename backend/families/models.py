...
class FamilyThread(models.Model):
    """
    Разговор родителя с учителем о ребёнке.

    Предмет — **тройка**: родитель, учитель и ребёнок. Каждая часть заработана
    отдельно:

    * **родитель**, а не «родители», — мама и папа пишут порознь, и сваливать
      их в один тред значило бы показывать каждому чужую переписку;
    * **учитель**, а не курс: родитель говорит с человеком. У ребёнка шесть
      курсов и один классный руководитель, и «разговор про математику» —
      это разговор с тем, кто её ведёт;
    * **ребёнок**, потому что детей у родителя несколько, и «как у него дела»
      без имени не вопрос.

    Форма повторяет `works.Thread` намеренно: тред заводится по требованию, у
    сообщения есть автор, а «нет разговора» отличается от «спросили и не
    ответили» самим наличием строки. Своей моделью, а не полем в том треде,
    потому что предмет другой: там задача и ученик, тут ребёнок и взрослые.
    """
    ...
    def __str__(self):
        return f"{self.parent} ↔ {self.teacher} о {self.child}"


class FamilyMessage(models.Model):
    """
    Реплика в разговоре. Строка журнала, а не правимая запись: сказанное
    родителю не редактируется задним числом — по той же причине, по какой не
    редактируется ответ ученика.
    """
