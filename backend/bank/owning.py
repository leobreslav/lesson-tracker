...
class OwnedQuerySet(models.QuerySet):
    def visible_to(self, user):
        ...
        **У администратора школы исключение**, и оно исправляет старую
        непоследовательность: право править школьное и личное у него было с
        самого начала (`writable_by`), а видимости не было — то есть ветка
        была недостижима: объект не находился раньше, чем спрашивалось право.
        Наружу это вылезло на предложениях: он разбирал сообщение об опечатке
        в задаче, которую не мог открыть.

        Расширена именно **видимость**, а не сужено право, потому что те же
        тексты он и так видит, открывая работы курсов своей школы: содержание
        чужих курсов он чинит с тех пор, как это правило ввели. Прятать их на
        полке значило бы прятать то, что уже показано в другом месте.
        """
        ...
        if user.is_school_admin:
            return self.filter(
                models.Q(school__isnull=True) | models.Q(school=user.school)
            )
