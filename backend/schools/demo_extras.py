...
def build(school, courses, people, students, *, log=print):
    ...
    # Эталон и методист заводятся на **другом** курсе: «Grade 6 Algebra» —
    # курс, на котором показывают план в его начальном, неутверждённом
    # состоянии, и посеянный эталон это состояние менял бы. Набору всё равно,
    # на каком курсе показать утверждённый план.
    ...
    # причине: план «Grade 6 Algebra», оставленный посреди хода, менял бы его
    # начальное состояние. Курс выбран
...
def _plan_walk(course, teacher):
    ...
    наборе, где его никто не начинал, второй кнопки хода не видно вовсе —
    человеку, смотрящему демо, её не увидеть.
...
# Длина школьного дня демо-школы. Больше, чем звонков: отработки и замены
# набор ставит седьмым и восьмым часом, и день обязан их вмещать.
DAY_LESSONS = 8
...
def _bells(school):
    """
    Школьный день демо-школы: восемь уроков, из них шесть со звонками.

    Восемь, а не шесть, потому что отработки и замены набор ставит седьмым и
    восьмым часом (`free_number(start=7)` в посеве): день, обрезанный по
    звонкам, отменил бы половину того, ради чего эти часы и посеяны.

    А звонки заполнены не у всех восьми — и это не недосев, а ровно то
    состояние, которое приложение считает рабочим: строка без времени
    показывается в сетке голым номером. Демо-набор обязан показывать его
    хотя бы раз, иначе его никто не увидит до первой живой школы.
    """
    from schedule.services import set_school_day
    ...
    return f"звонков {len(BELLS)} на {DAY_LESSONS} уроков"
