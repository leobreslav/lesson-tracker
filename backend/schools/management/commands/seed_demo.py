...
# школ и обращения пользователей. Без него в посеянной базе их **некому открыть** —
...
class Command(BaseCommand):
    ...
    def flush(self):
        ...
        # После пересева это выглядело как список сообщений от
    ...
    def developer(self):
        """
        Суперпользователь посеянной базы. Школы у него нет — и это не пропуск.
    ...
    def year(self, school, *, rich=False):
        ...
        Даты базового набора зашиты нарочно: посев воспроизводим, и
        «первый понедельник» у него всегда один и тот же. Крупному набору,
    ...
    def attach(self, school, email, *, root=False):
        ...
        для пересеваемой базы. Разделы «Школы» и «Обращения» открыты только
        суперпользователю, а стать им иначе после пересева нечем: `bootstrap`
