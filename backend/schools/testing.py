...
def last_workday(day=None):
    """
    Ближайший будний день не позже этой даты; по умолчанию — не позже сегодня.

    Нужен там, где набор строит часы «неделю назад» кратно семи. Такие даты
    попадают в тот же день недели, что и сегодня, — и в субботу весь набор
    оказывается на выходных. Сам `make_slot` этого не замечает, календарь его
    не спрашивают; а вот копирование спрашивает: в цели не находится ни
    одного учебного дня, оно честно не создаёт ничего и отвечает успехом.

    Так и вышло: `test_copying_into_the_closed_past_is_refused` ждал отказа,
    получал 200 и падал — но только по субботам и воскресеньям, то есть
    ночным прогоном на выходных и никогда в рабочий день.
    """
    from django.utils import timezone

    # имя `day`, а не `date`: в модуле уже есть импортированный тип `date`,
    # и параметр с тем же именем закрыл бы его собой
...
def live_year(school, name="живой год"):
    """
    Год вокруг сегодня — для правил, которые считают прошедшее.

    Зашитый 2026/2027 стоит в будущем, и «записать можно только прошедший
    час» на нём не проверить: прошедших часов там не бывает вовсе. Год,
    посчитанный от текущей даты, не протухает — та же причина, по которой
    так устроен крупный демо-набор.
    """
    from django.utils import timezone
...
def make_work(teacher, course, *, title="Контрольная", opens=None, closes=None, **fields):
    """
    Работа курса. Окно по умолчанию — открытое прямо сейчас, и работа выдана.
    ...
    Выдана по той же причине. В базе умолчание обратное — заведённая работа
    ещё пишется и ученику не видна, — но у фикстуры вопрос другой: почти
    каждый тест про работу спрашивает про **выданную**, а невыданная нужна
    ровно там, где её и проверяют, и там она пишется явно
    (`make_work(..., is_released=False)`).

    Разница эта уже стоила одного разбора: без неё 78 тестов упали разом, и
    все — на том, что ученик перестал видеть работу.
