...
ACTIONS_WITHOUT_ID = {
    ...
    "plannode-redo": "шаг вперёд по ленте плана: владелец в строке запроса",
    "slot-redo": "то же у расписания: курс находится по самому свежему снимку",
}
...
class MigrationTests(SimpleTestCase):
    ...
    разработки, а этого ни один тест увидеть не может — `manage.py test`
    поднимает базу с нуля и мигрирует её целиком.
