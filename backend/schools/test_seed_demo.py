...
class AttachTests(TestCase):
    ...
    def test_root_makes_the_named_account_a_superuser(self):
        """
        Стать суперпользователем в пересеянной базе иначе нечем.
...
class PlainSeedYearTests(TestCase):
    ...
    def test_the_plain_seed_keeps_its_fixed_year(self):
        """
        Маленький набор воспроизводим: его даты от посева к посеву одни.

        Живой год нужен крупному набору и только ему: маленький сеют,
        чтобы смотреть на одно и то же, и «первый понедельник» в нём
        всегда один.
        """
        seed()
