...
test('исписанное поле тетрадных клеток не сбивает поиск шапки', () => {
  /*
   * На бумаге поле записи есть всегда и занимает пять шестых листа, а дети в
   * нём пишут — то есть чистым оно не бывает. Фикстура же его не рисовала
   * вовсе, и все замеры шли по листу, какого у школы не будет.
   *
   * Опасность несимметрична. Обе защиты — «стена вертикалей это поле, а не
   * сетка» и «над сеткой строка имени, а не поле» — узнают поле **по его
   * печатным линиям**. Почерк их частично закрывает, то есть делает защиту
   * мягче, а не строже: кривой кандидат может проскочить там, где на чистом
   * листе не проскакивал.
   */
  for (const written of [0, 8, 20]) {
    const found = extractHeader(drawSheet({ field: true, fieldInk: written, ink: 6 }))

    assert.ok(found, `при ${written} исписанных строках шапка не нашлась вовсе`)
    assert.ok(
      found.score >= ENOUGH_LINES,
      `при ${written} исписанных строках границ ${found.score} из нужных ${ENOUGH_LINES}`,
    )
    // и полоска взята именно с шапки, а не с поля записи
    assert.equal(nameRowIsClear(found.strip), true, `полоска уехала на поле записи`)
  }
})
