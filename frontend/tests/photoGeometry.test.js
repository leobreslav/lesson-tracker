...
test('страницы листаются по кругу, как и снимки', () => {
  // правило одно на оба листания намеренно: человек листает стопку, и ему
  // безразлично, набор это снимков или страницы одного PDF
  assert.equal(pageAt(7, 0, 1), 1)
  assert.equal(pageAt(7, 6, 1), 0)
  assert.equal(pageAt(7, 0, -1), 6)
})

test('документ из одной страницы листается сам в себя, а не в пустоту', () => {
  assert.equal(pageAt(1, 0, 1), 0)
  assert.equal(pageAt(1, 0, -1), 0)
})

test('нераскрытый документ отвечает нулевой страницей, а не null', () => {
  // страница ноль существует всегда, как только появится хоть одна: `null`
  // здесь пришлось бы проверять в каждом месте, где считают пометки
  assert.equal(pageAt(0, 0, 1), 0)
})

test('пометки берутся по своей странице, а чужие не берутся', () => {
  const marks = [{ page: 0, id: 1 }, { page: 3, id: 2 }, { page: 3, id: 3 }]

  assert.deepEqual(onPage(marks, 3).map((one) => one.id), [2, 3])
  assert.deepEqual(onPage(marks, 1), [])
})

test('пометка без страницы — это пометка снимка, то есть нулевая', () => {
  // так лежит всё, что размечено до того, как страницы появились: у снимка
  // страница одна, и молча потерять на нём проверку учителя нельзя
  assert.deepEqual(onPage([{ id: 1 }], 0).map((one) => one.id), [1])
  assert.deepEqual(onPage([{ id: 1 }], 2), [])
})

test('пометок нет вовсе — это пустой список, а не поломка', () => {
  assert.deepEqual(onPage(undefined, 0), [])
})
