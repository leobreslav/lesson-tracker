...
export default function Agenda({ views = null, onLoggedOut }) {
  ...
  return (
    <main className="page wide">
      ...
      {/*
        Пусто — это «курсов нет вовсе», а не «пролистали в август».

        Условие смотрело на курсы **показанного периода**, и стоило уйти
        стрелкой за границы учебного года, как расписание объявляло: «у вас
        пока нет курсов», предлагая завести первый. Неправда: курсы есть, в
        этой неделе у них просто не бывает уроков.

        Цена этой неправды оказалась не только словесной. Пустое состояние
        подменяло собой строки фильтра, то есть при листании через границу
        года один блок исчезал, а другой появлялся, и страница на этом
        менялась в росте — браузер прижимал прокрутку к новой высоте.
        Держалось это на том, что высоты случайно совпадали; третий
        переключатель совпадение сломал, и сторож «листание не уносит
        страницу в начало» нашёл заодно и саму подмену.

        Само пустое состояние при этом по-прежнему держится на том, что
        **показано**, а не на том, идёт ли запрос: пока оно пряталось на
        время загрузки, страница укорачивалась ровно так же.
      */}
      ...
      {/*
        Переключатели вида — своей строкой, а не в хвосте фильтра классов.

        Разное это по смыслу: слева выбирают, **что показать** (и список
        меняется от недели — в фильтре стоят только те курсы, у которых на
        этой неделе есть часы), справа — **как показать**, и это от недели
        не зависит вовсе.

        А началось с поломки: третий переключатель столкнул общую строку
        через порог переноса, и на неделе с меньшим числом курсов она
        распрямлялась обратно — страница при листании становилась на сотню
        пикселей короче, браузер прижимал прокрутку к новой высоте, и
        сторож «листание не уносит страницу в начало» справедливо падал.
        Пока переключателей было два, это просто не проявлялось.
      */}
      {classes.length > 0 && (
