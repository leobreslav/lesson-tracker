...
const SECTIONS = [
  ...
  // Журнал рядом с работами, а не внутри них: спрашивают его отдельно и
  // ходят в него часто — на собрании, на педсовете, в конце четверти, — а
  // «Работы» отвечают на другой вопрос, «что я задал и что проверить»
  { to: '/journal', key: 'journal', needs: 'classes' },
  // переписка ни от чего не зависит: коллеге пишут и без курсов
  { to: '/talks', key: 'talks', needs: null },
  // задачник ни от чего не зависит: библиотека читается и без курсов
  { to: '/bank', key: 'bank', needs: null },
  // Закладки не зависят ни от чего и не гаснут никогда: это личный стол, и
  // он осмыслен ровно в тот день, когда в школе ещё ничего не настроено, —
  // ссылку на методичку кладут раньше, чем заводят первый курс
  { to: '/bookmarks', key: 'bookmarks', needs: null },
  // «Курсы» из бара убраны: они живут в разделе «Школа», где их и правят, а
  // учителю список курсов и так виден чипами на своих страницах —
  // расписании, плане и работах.
  //
  // «Учебный год» уехал туда же и по тому же доводу — и довод оказался верен
  // только для администратора. У учителя раздела «Школа» нет вовсе, то есть
  // двери в календарь не осталось ни одной: маршрут открыт, сервер отдаёт
  // (год читает вся школа, правит администратор), страница умеет режим
  // чтения — а нажать негде. Спрашивают же его постоянно: когда каникулы,
  // сколько учебных недель осталось, почему в сетке серая среда.
  //
  // Стоит последним из обычных: смотрят его редко, а первые пункты — то,
  // что открывают каждый день. Не гаснет ни от чего: без года эта страница
  // как раз и говорит, что года нет и завести его может администратор
