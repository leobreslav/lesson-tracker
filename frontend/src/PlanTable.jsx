 ...
 * эталон; таблица — вёрстка, которую переделывали трижды. Пока они жили в одной
...
export default function PlanTable({
  ...
  /**
   * Смена вида меняет и место, куда встанет строка, — если открывшая форму
   * кнопка назвала два места.
   *
   * У «+» в шапке темы они разные: урок встаёт **первым внутри** темы, тема —
   * **следом за всем блоком**. У остальных мест место одно на оба вида
   * («вставить после» режет блок ровно там, где стоит), и тогда меняется
   * только флаг.
   *
   * Хранить оба якоря в самой форме, а не пересчитывать при нажатии, надо
   * потому, что тумблер щёлкают туда-обратно: пересчёт «от текущего» после
   * первого же возврата дал бы не то место, с которого начали.
   */
  const withKind = (state, isSection) => ({
    ...state,
    is_section: isSection,
    ...((isSection ? state.asSection : state.asLesson) ?? {}),
  })

  const addForm = () => (
    <form className="plan-add-form" onSubmit={finishAdd} ref={formRef}>
      {/* Что заводим — урок или тему. Спрашивается везде, кроме клика по
          свободному часу: там вопрос уже задан самим местом — это пустой
          час, в него ставят урок. Раньше вид задавала кнопка, и форм было
          как бы три: с тумблером, «только урок» и «только тема». */}
      {!adding.fixedKind && (
        <Switch
          label={t('plan.addKind')}
          value={adding.is_section}
          disabled={busy}
          onChange={(value) => changeAdding(withKind(adding, value))}
          options={[
                      ...
                      <button
                        ...
                        onClick={() =>
                          add({
                            ...
                            // Тумблер здесь тоже есть, но «Тема» значит не
                            // «внутрь» — тема в тему не кладётся, — а
                            // «следом за всем этим блоком». Якорем берётся
                            // последний урок темы: разрез по нему оставляет
                            // хвост пустым, и новая тема встаёт сразу за
                            // блоком (`services.split_at`).
                            //
                            // Якорь при этом обязан быть **тем местом, где
                            // форма рисуется**: `addFormFor` ищет пару
                            // «родитель и якорь», и `parent: null` с уроком
                            // внутри темы не совпал бы ни с одним местом —
                            // форма исчезала бы при щелчке по тумблеру.
                            // Поэтому родитель остаётся темой: форма встаёт
                            // после последнего её урока, там же, где
                            // появится сама тема.
                            //
                            // У пустой темы резать нечего, и якорь — она
                            // сама, на верхнем уровне: `split_at` кладёт
                            // новую тему сразу за ней.
                            asLesson: {
                              parent: node.id,
                              before: node.children?.[0]?.id ?? null,
                              after: null,
                            },
                            asSection: node.children?.length
                              ? {
                                  parent: node.id,
                                  before: null,
                                  after: node.children.at(-1).id,
                                }
                              : { parent: null, before: null, after: node.id },
                          })
                        }
                      >
                        +
