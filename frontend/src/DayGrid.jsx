 ...
 * решает тот, кто смотрит (`dayAxis.js`): ищущий, кем закрыть окно, видит
 * учителей; ищущий свободное помещение — кабинеты; классный руководитель —
 * классы. Данные при этом одни и те же, и здесь про ось не знают ничего:
 * столбцы и раскладку считает страница.
 *
 * Столбцами были **курсы**, и с них всё началось — стопку разворачивали
 * именно в них. Ось курсов убрана по просьбе: столбец «9Б Алгебра»
 * пересказывал название, уже стоящее в фильтрах, и не отвечал ни где урок
 * идёт, ни кто его ведёт. Имя курса переехало в клетку, где его и ищут.
 *
 * **Клетка держит стопку, и это не уступка.** Ни одна из оставшихся осей не
 * обещает одного часа в клетке: в делимом зале два занятия разом норма, у
 * заменяющего учителя два часа в одном номере тоже бывают, а курс из кусков
 * четырёх классов стоит в четырёх столбцах. Сетка, умеющая показать один
 * час, молча прятала бы второй.
 *
