 ...
 *
 * Порядок черт хронологический: у каждой есть своя дата, и ленту читают
 * сверху вниз как календарь, а не как оглавление. Неделя строкой не
 * является вовсе: она рисуется скобкой в левой колонке
...
export function stitchLayout(rows, ribbon, today = null) {
  ...
  const stitched = rows.map((row) => {
    ...
    if (today && slot && !past && !todayDone) {
      todayDone = true
      before.push({ kind: 'today', date: today })
    }

    /*
     * Черты между двумя строками стоят по датам, а не по «крупности».
     *
     * Порядок был фиксированный, от крупного к мелкому — терм, каникулы,
     * сегодня, — и врал он дважды. Сперва на границе четвертей: зимние
     * каникулы кончаются 6 января, третья четверть начинается 7-го, а
     * синяя черта каникул стояла **под** заголовком четверти. Потом на
     * начале года: сегодня 18 августа, первый урок 7 сентября, и красная
     * черта «сегодня» оказывалась **ниже** заголовка терма, который ещё
     * не начался.
     *
     * Лечится это не третьим исключением, а тем, что у каждой черты есть
     * своя дата: ленту читают сверху вниз как календарь, значит и черты
     * идут по датам. Сортировка устойчивая — при совпадении дат порядок
     * остаётся прежним.
     */
    if (before.length > 1) {
      const when = (mark) => mark.date ?? mark.start ?? slot?.date ?? ''
      before.sort((a, b) => (when(a) < when(b) ? -1 : when(a) > when(b) ? 1 : 0))
    }
