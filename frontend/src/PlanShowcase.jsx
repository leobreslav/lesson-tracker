 ...
 *
 * Запросов отсюда не уходит ни одного, и это не изменилось с появлением
 * переключателя видимости: витрина показывает и зовёт — `onPick`, `onCreate`,
 * `onPublish`. Что делать с выбором, знает страница, и знает по-разному: у
 * курса свой адрес, у заготовки свой, а публикацию она к тому же обязана
 * догнать перечитыванием двух списков сразу. Держать это здесь значило бы
 * завести второй ответ на вопрос, у которого уже есть один.
 */
...
export default function PlanShowcase({
  ...
  onPick,
  onCreate,
  /* переключить видимость своей записи на полке; без него плашка остаётся
     невидимой вовсе — показывать состояние, которое нельзя изменить, здесь
     нечем: у витрины нет ни окна, ни панели действий, куда бы вёл отказ */
  onPublish = null,
  busy = false,
}) {
        ...
        {areas.map((area, index) => (
          <section key={area.key} className="showcase-area">
            ...
            */}
            ...
            {area.items.length === 0 ? (
              ...
              <ul className="showcase-list">
                {area.items.map((item) => (
                  <li key={`${item.kind}-${item.id}`} className="showcase-line">
                    {/*
                    ...
                    */}
                    ...
                    >
                      ...
                      {/*
                        Пометка внутри строки — только та, что про действие, а
                        не про свойство записи: «ждёт решения» зовёт открыть
                        курс сейчас.
                      */}
                      {item.waiting && (
                    ...
                    {/*
                      Видимость своей записи — плашка-кнопка, и стоит она
                      **рядом** со строкой, а не внутри неё.

                      Внутри её и быть не может: строка целиком — кнопка, а
                      кнопка в кнопке невалидна и нажимается непредсказуемо.

                      Стояла тут неподвижная пометка «черновик», и отвечала
                      она на половину вопроса: черновик было видно, а
                      опубликованное молчало — то есть «видит вся школа» и
                      «я не знаю, что с этой записью» выглядели одинаково,
                      пустым местом. Названы теперь обе стороны.

                      И обе нажимаются: это идиома `button.badge` — нажатие
                      здесь не действие рядом с состоянием, а само состояние,
                      как отметка в журнале. Дверь к переключению была одна и
                      неблизкая: открыть запись и найти тумблер в панели
                      действий; со списка своих заготовок это четыре шага
                      ради одного слова.

                      Только у своих записей на полке. У чужой вопроса нет
                      вовсе: чужой черновик до нас не доезжает, а
            ...
            */}
            ...
            {/*
              Черта под колонкой — вторая половина креста; первая,
              вертикальная, нарисована в стилях слоем поверх сетки.

              Живёт она **внутри** третьего яруса, вместе со списком и
              кнопкой, и по той же причине, что и они: ярусов у области ровно
              три, и четвёртый ребёнок увёл бы колонку в дорожку, которой у
              соседки нет.

              Стоит она под содержимым **своей** колонки, а не общей строкой
              на всю ширину. Колонки наполнены по-разному — планов курсов
              может быть десять, а заготовок одна, — и общая черта у той
              колонки, что короче, висела бы под пустотой: делила бы не своё
              и чужое, а свой список и воздух под ним. Поэтому черт две, и
              стоять они могут на разной высоте.

              `axis` помечает ту из них, что делит **ось**, а не просто две
              соседние области. На широком экране это различие не нужно —
              колонок две, и под каждой своя черта; на узком колонка одна, и
              видимой остаётся только помеченная. Читается по самой оси, а не
              по номеру: порядок областей задан в `AREAS` одним списком, и
              привязка к номеру разъехалась бы с ним молча.
            */}
            {area.mine && (
              <div
                className={`showcase-divider${
                  areas[index + 1] && areas[index + 1].mine !== area.mine
                    ? ' axis'
                    : ''
                }`}
                aria-hidden="true"
