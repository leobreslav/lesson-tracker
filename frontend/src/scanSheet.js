 ...
 *
 * Различает их то, что **над** сеткой на бланке напечатана строка имени:
 * подписи, линейки из точек и рукописное между ними. Сплошных вертикалей там
 * нет ни одной. У съехавшей полоски на этом месте оказывается тетрадное
 * поле — стена линий через каждые пять миллиметров.
 *
 * Считаются поэтому не «тёмные столбцы», а столбцы, тёмные **во всю высоту**
 * полосы. Разница не придирка: буквы имени и штрихи почерка дают сколько
 * угодно тёмных столбцов, и по среднему они неотличимы от линеек — первая же
 * проверка по среднему отбросила две трети годных страниц. Линия проходит
 * полосу насквозь, буква нет.
 *
...
export function nameRowIsClear(strip) {
  ...
  for (let x = 0; x < width; x += 1) {
    let dark = 0
    for (let y = 0; y < bottom; y += 1) if (gray[y * width + x] < level) dark += 1
    const solid = dark >= bottom * 0.8
    if (solid && !inLine) lines += 1
    inLine = solid
  }
  return lines <= RULED
}
