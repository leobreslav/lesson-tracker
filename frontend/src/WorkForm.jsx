...
/**
 * Настройки работы: название, задание, файлы, окно времени, попытки,
 * система оценивания, показ отметки.
 *
 * **Форма живёт отдельно от того, во что её вставили, и вставляют её в два
 * разных места.** Заводят работу окном (`WorkDialog`), правят — страницей
 * (`WorkEdit`). Разделение это не про вид, а про то, сколько на экране
 * всего: при заведении заполняют название и две даты, а правят работу, у
 * которой уже есть задание с картинками, приложенные файлы и настроенное
 * оценивание, — и всё это в окне поверх страницы читалось в щёлку.
 *
 * Копии формы для второго места быть не может: полей здесь пятнадцать, у
 * половины своя подсказка, и две копии разошлись бы в первую же правку —
 * молча, как обычно расходятся копии в этом проекте.
 *
