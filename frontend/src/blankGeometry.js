 ...
 *
 * Сверху не режется вовсе — берётся **от верха листа** до низа сетки. Резать
 * там нечего: над строкой имени напечатано только поле, зато промах кропа
 * вверх стоит дорого. Съехала гомография на несколько миллиметров — и у
 * строки имени срезается верх букв, а выглядит это как «модель стала хуже
 * читать почерк». Восемь лишних миллиметров бумаги модели не мешают, а
 * стоят они +27% к картинке полоски, то есть доли цента на пачку.
 *
 * По бокам запас в пару миллиметров остался: печать и резка бумаги гуляют, а
 * обрезанная по краю клетка читается хуже, чем клетка с полем вокруг.
 */
export const HEADER = { x: 10, y: 0, width: 190, height: 38 }
