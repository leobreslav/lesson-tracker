...
/**
 * Шаг назад по ленте состояний плана; с номером — сразу в это состояние.
 *
 * Шагов столько, сколько снимков: отменяется не только последнее действие.
 * Номер нужен «вернуть как было» после чужой правки — это тот же указатель,
 * просто поставленный сразу, а не десятком нажатий.
 */
...
/**
 * Шаг вперёд: вернуть то, что только что отменили.
 *
 * Своей ручкой, а не флагом у отмены: это разные движения, и на кнопках они
 * названы разными словами. Пока движение было одно, второе нажатие «Отменить»
 * возвращало отменённое — и отменить два действия подряд было нельзя.
 */
export const redoPlan = (owner) =>
  request(`/api/plan/redo/?${ownerQuery(owner)}`, { method: 'POST', body: {} })
