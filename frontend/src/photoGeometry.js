...
/**
 * Соседняя страница внутри документа — по кругу, как и снимок в наборе.
 *
 * Правило одно на оба листания намеренно: человек листает стопку, и ему
 * безразлично, чем она стала внутри системы — набором снимков или страницами
 * одного PDF. После последней ждут первую, а не упор.
 *
 * Документ без страниц — это документ, который ещё не открылся: ответ ноль,
 * а не `null`, потому что страница ноль существует всегда, как только
 * появится хоть одна.
 */
export function pageAt(count, current, direction) {
  if (!count || count < 1) return 0
  return (((current + direction) % count) + count) % count
}

/** Пометки этой страницы. У снимка страница одна, и она нулевая. */
export function onPage(marks, page) {
  return (marks ?? []).filter((one) => (one.page ?? 0) === page)
}
