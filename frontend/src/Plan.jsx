...
export default function Plan({ user, onLoggedOut, template = null }) {
  ...
  useEffect(() => {
    ...
    /*
     * Карточка полки — только когда мы на ней. Курсы спрашиваются **всегда**.
     *
     * Раньше на полке курсы не спрашивались вовсе: селектора тут не было, и
     * четыре запроса ради него выглядели чистой тратой. Селектор появился, и
     * с ним появился выход: с заготовки можно уйти в курс, не возвращаясь
     * через библиотеку. Ради этого списки и нужны — те же самые, что на
     * стороне курса, иначе селект был бы двумя разными селектами, смотря
     * откуда зашли.
     */
    if (onShelf) {
   ...
   * а ошибка в консоли на каждом открытии чужого плана прячет настоящие.
   */
   ...
   *
   * Спрашивают это из трёх мест: списком в окне полки, плашкой на витрине и
   * на самой открытой записи — в шапке и в ряду действий.
   *
   * **Мимо `run`, и это главное в нём.** `run` поднимает общий `busy`,
   * перечитывает дерево плана и только потом отпускает; здесь же меняется
   * одно булево поле, до дерева не достающее вовсе. Наружу общий `busy`
   * выходил морганием: на странице заготовки вздрагивала синяя «Добавить
   * тему или урок», а на витрине — **все** карточки планов разом, потому что
   * у каждой `disabled={busy}`. Со стороны это читается как «экран
   * перезагрузился», хотя перерисоваться должна была одна плашка.
   *
   * Списки после ответа не перечитываются, а правятся точечно. Перечитывание
   * было вторым источником того же моргания и стоило двух лишних запросов;
   * значение при этом берётся **из ответа сервера**, а не из намерения, —
   * иначе это была бы вера в то, что запись прошла.
   *
   * Править надо оба места сразу: список кормит витрину и окно полки, а
   * карточку читают шапка и ряд действий. Обновив одно, второе оставили бы
   * показывать прежнее — то есть переключатель щёлкал бы вхолостую ровно там,
   * куда человек в этот момент и смотрит.
   */
  const publishTemplate = (item, published) => {
    setError(null)

    return updateTemplate(item.id, { is_published: published })
      .then((updated) => {
        const value = updated?.is_published ?? published
        const patch = (row) =>
          row && row.id === item.id ? { ...row, is_published: value } : row

        setTemplates((list) => list.map(patch))
        setShelfCard(patch)
      })
      .catch(handleError)
  }
  ...
  /**
   * Мои заготовки — пятая группа селекта, и она не курс.
   *
   * Заготовка с полки правится тем же экраном, что план курса, но дороги
   * назад к ней не было: закрыл — и возвращаться надо через план курса, меню
   * «Библиотека», окно полки и кнопку «Править». Четыре шага ради того, что
   * человек делает подряд, и все четыре — не про его работу, а про то, где
   * лежит дверь.
   *
   * Дверь тут одна и та же, что у курса: «чем сейчас занимаемся». Поэтому и
   * список один, а не второй селект рядом.
   *
   * Приставка `t` у id — чтобы шаблон №7 не оказался курсом №7. Различать их
   * по группе нельзя: наружу из селекта приезжает одно значение.
   *
   * Учитель и предмет приписываются свои, а не пустые: селект умеет сужаться
   * по учителю, и заготовка без него вычищалась бы вместе с чужими курсами —
   * то есть исчезала бы ровно тогда, когда человек ищет **своё**. Имя берётся
   * готовым (`author_name`), а не склеивается тут из профиля: у курсов оно
   * приезжает от сервера, и вторая склейка разошлась бы с ней на первом же
   * человеке без фамилии.
   */
  const asTemplate = (item) => ({
    id: `t${item.id}`,
    name: item.title,
    year: null,
  ...
  /**
   * Учебный год открытого курса — названием, из какого бы списка он ни
   * приехал.
   *
   * Списка два, и год в них выражен по-разному: свои и школьные курсы несут
   * **номер** года (`SchoolYear.id`, разворачивается по `years`), а
   * поднадзорные приезжают строкой прогресса, где год уже **название**.
   * Разница эта не наша выдумка и не ошибка — просто ручки разные, — но
   * шапке нужен один ответ, и приводится он здесь, а не в разметке.
   */
  const openYear = (() => {
    if (onShelf || !openItem) return null
    const known = yearById.get(openItem.year)
    if (known) return known.name
    return typeof openItem.year === 'string' ? openItem.year : null
  })()

  /**
   * В какой роли человек пришёл к этому плану — и почему это сказано словом.
   *
   * Селект отвечает на «чем сейчас занимаемся», и отвечает верно, но
   * **только пока открыт**: у него пять групп, и они-то и есть роли — свои
   * курсы, присланные на подпись, поднадзорные, курсы школы, заготовки.
   * Закрылся — и от всего этого остаётся одно имя, у которого группы не
   * видно. «7Б Физика» из «Мои курсы» и «7Б Физика» из «Курсов школы»
   * выглядят одинаково, а экран под ними разный: у первого таблица с
   * правкой, у второго чтение.
   *
   ...
   *
   * У **своей** заготовки роли нет вовсе, и это не пропуск. Править её
   * вправе только автор, а автор тут я — роль вырождается в видимость, и
   * вопрос остаётся один: видит ли это кто-нибудь, кроме меня. Отвечает на
   * него не роль, а плашка видимости — **та же**, что в списках витрины и
   * окна полки.
   *
   * Пока отвечала роль, ответов было два и оба кривые. «Черновик, видите
   * только вы» говорил про видимость, а сменявшее его «правка доступна» —
   * про право правки, то есть про другой вопрос; а рядом, в панели
   * действий, стоял ещё и тумблер «Всей школе · Только мне». Один вопрос,
   * три разных формы на одном экране, и ни одна не совпадала с тем, как то
   * же самое показано в списках.
   *
   * У **чужой** записи вопрос обычный, тот же, что у чужого курса: почему я
   * не могу править и чей это план. Поэтому она и отвечает как чужой курс —
   * «только чтение» с именем автора, — а не своей видимостью: «на общей
   * полке» у чужой записи было бы правдой, ничего не объясняющей.
   *
   * `null` — это «ничего не выбрано» либо «называть нечего».
   */
  const role = onShelf
    ? shelfForeign
      ? 'reading'
    ...
    : !classId
      ...
      : /* решать — только по поднадзорному, и только пока есть что решать:
           у своего курса запрос решается ссылкой в строке состояния */
        supervising && supervisedRow?.review?.status === 'pending'
        ? 'deciding'
        : supervising
          ? 'reading'
          : 'editing'
  ...
  return (
    <main className="page wide">
      {/*
        Дорога назад — первой строкой и у самого левого края.

        Стояла она в одном ряду с заголовком, справа от роли, и там читалась
        как ещё одно действие над планом — рядом с «На утверждение» и
        тумблером вида. А это не действие над планом, а выход из него: то
        же, что «назад» в браузере, только внутри раздела. Такие ставят
        первыми и слева — там их ищут глазами, не читая строку до конца.

        Ведёт она туда же, куда пункт «Учебный план» в баре: на `/plan`, то
        есть к витрине.
      */}
      {openName && (
        <button
      ...
      <header className="page-header">
        {/*
          ...
          контроле, у которого не видно ни вида, ни того, выбрано ли
          что-нибудь вообще. Контрол с одним именем внутри читается подписью.

          На его месте — то, чего не было: **чей это план, сказано
          построчно**. Одной строкой «Курс: 7Б Физика» это было, и строка
          отвечала лишь на половину вопроса: у плана курса есть ещё учебный
          год и ведущий, и оба нужны как раз тогда, когда планов много, —
          «7Б Физика» бывает и прошлогодней, и чужой. Теперь каждый ответ на
          своей строке.

          У заготовки вопросы другие, и заголовок у неё поэтому свой: она не
          про курс вовсе, а про программу без класса и года. Сказать это
          подписью под общим заголовком мало — человек, пришедший по ссылке,
          читает заголовок и решает, туда ли попал.
        */}
        {/* Заголовок и подписи — одной колонкой: подписи уточняют заголовок,
            а не стоят рядом с ним. Сама шапка выкладывает детей в ряд (справа
            от заголовка живёт вид), поэтому колонка объявлена своей обёрткой,
            а не наклоном самой шапки. */}
        <div className="plan-title">
        <div className="plan-heading">
          <h1>{t(onShelf ? 'plan.head.fromLibrary' : 'plan.title')}</h1>
          {/*
            Плашка называет **роль, и только её**.

            Имя хозяина стояло тут же («Пётр Петров · только чтение»), и
            стояло не зря: у чужого плана спрашивают не «что нельзя», а «чей
            он и к кому идти». Отвечать на это плашке пришлось потому, что
            больше отвечать было нечем — под заголовком стояло одно имя
            плана. Теперь там строка «учитель:» у курса и «автор:» у полки,
            то есть ответ дан, и повторённый в плашке он читался бы вторым
            фактом о том же.
          */}
          {role && (
            <span className={`open-role ${role}`}>{t(`plan.role.${role}`)}</span>
          )}
          {/*
            У своей записи на полке на месте роли стоит видимость — та же
            плашка, что в списках витрины и окна полки, и она же
            переключатель.

            Стояла тут роль, а переключала видимость отдельный тумблер в
            панели действий: один вопрос, два места и три разные формы на
            одном экране. Плашка отвечает и переключает разом, ровно как в
            списках, — а список и есть то, откуда сюда приходят.
          */}
          {onShelf && !shelfForeign && shelfCard && (
            <VisibilityBadge
              published={Boolean(shelfCard.is_published)}
              busy={busy}
              onChange={(next) => publishTemplate(shelfCard, next)}
            />
          )}
        </div>
        {openName && (
          <div className="hint plan-about">
            {onShelf ? (
              <>
                <span className="open-name">
                  {t('plan.head.plan', { name: openName })}
                </span>
                {shelfCard?.author_name && (
                  <span>
                    {t('plan.head.author', { name: shelfCard.author_name })}
                  </span>
                )}
              </>
            ) : (
              <>
                <span className="open-name">
                  {t('plan.head.forCourse', { name: openName })}
                </span>
                {openYear && <span>{t('plan.head.year', { year: openYear })}</span>}
                {/*
                  Ведущий — строкой, только когда он есть. «Учитель: —» у
                  курса, которому не раздали нагрузку, добавляет прочерк там,
                  где вопрос не задавали: шапка отвечает на «чей это план», а
                  не перечисляет поля курса. Что нагрузка не раздана, говорят
                  там, где её раздают.
                */}
                {openItem?.teacher && (
                  <span>{t('plan.head.teacher', { name: openItem.teacher })}</span>
                )}
              </>
            )}
          </div>
        ...
        {/*
          В шапке от утверждения остался **вид**, а не действие.

          Собрано оно было здесь целиком — отправка, состояние и сравнение,
          — и собрано не зря: до того эти три половины одного разговора
          лежали в трёх разных концах экрана. Ошибка была не в том, что их
          свели вместе, а в том, куда: шапка отвечает на «что открыто», и
          кнопка, отправляющая план человеку, читалась в ней как свойство
          плана. Рядом с ней стоял выход из плана, и обе смотрелись одинаково.

          Поэтому действие и его состояние уехали в панель, к остальным
          действиям над планом (`plan-approval` там же), а здесь остался
          тумблер: сравнение — не действие, а **другой вид страницы**, в
          котором самой панели не показывается вовсе. Разговор от этого не
          разорван: обе половины, которые про «что сделать и что вышло»,
          стоят рядом.
        */}
        {/* Тумблер, а не кнопка: это выбор из двух видов, и оба надо
            назвать. Кнопка «Сравнить с эталоном» говорила только про один из
            них, а второй, обратный, приходилось искать глазами внутри
            открывшегося экрана — не там, где включали.

            Условие тут одно на всё, а было два вложенных: внешнее держало
            группу утверждения, внутреннее — сам тумблер. Группы больше нет,
            и пустая обёртка при неутверждённом плане занимала бы в шапке
            место (`margin-left: auto`) ничем. */}
        {!supervising && !onShelf && classId && baseline?.approved && (
      ...
      {/*
        Куда идут правки — сказано словами, и с обеих сторон одинаково.

        Экран один на две вещи: план курса и заготовку с полки. Отличаются
        они молча — по наличию дат, — и человек, зашедший поправить строку,
        мог править не то, что думал: заготовка сохраняется прямо в
        библиотеку, а план курса до библиотеки не доходит вовсе.

        Строка **постоянная и симметричная**, и оба свойства нужны. Разовое
        уведомление читают один раз, а вопрос «а это точно курс?» возникает
        каждый раз. Одна строка на одной стороне читалась бы как
        предупреждение об особом случае; две говорят, что случая два, и оба
        обычные.

        Случая с тех пор три, и третий — чужой план. Строки у него не было
        вовсе: правок тут нет, обещать их нельзя, и на этом рассуждение
        останавливалось. Но вопрос человека к чужому экрану не «куда идут
        мои правки», а «почему я не могу править» — и молчание отвечало на
        него хуже всего: кнопки просто отсутствовали, без причины. Теперь
        сказано и это, и заодно то, ради чего сюда приходят, — что чужой
        план забирают себе выгрузкой.

        Не показывается строка ровно там, где неверна: пока ничего не
        выбрано, говорить не о чем — там своё пустое состояние.
      */}
          ...
          <section className="panel plan-tools">
            <div className="actions wrap">
              ...
              */}
              ...
              {/*
                Видимость — второй раз, рядом с «Файлом», и это дубль
                намеренный.

                Плашка в шапке отвечает на вопрос «что открыто» и видна не
                читая панель — за этим она там и стоит. Но переключают
                видимость **действием**, а действия человек ищет здесь, в
                ряду рядом с «Файлом»: там же, где у курса «На утверждение»,
                и вопрос у них общий — кто ещё это увидит.

                Раньше в этих двух местах стояли два **разных** органа:
                неподвижная роль наверху и тумблер внизу. Теперь орган один,
                показан дважды и в обоих местах ведёт себя одинаково —
                нажатие переключает, подпись называет текущее состояние.

                Ростом он тут с кнопку (`control`), а не с плашку: в ряду
                контролов плашка высотой в строку текста прижималась бы низом
                к кнопкам и читалась съехавшей — ровно то, о чём правило
                «строка формы» и сторож рядов.
              */}
              {onShelf && !shelfForeign && shelfCard && (
                <VisibilityBadge
                  control
              ...
              {/*
                «На утверждение» — здесь, среди действий над планом.

                Стояла она в шапке, и место было плохое дважды. Шапка
                отвечает на «что открыто», а это действие, и притом
                отправляющее написанное другому человеку; рядом с ней стоял
                выход из плана, и обе читались одинаково. А кроме того,
                отправка — родня импорту и полке: всё это про то, что с
                планом сделать, и живёт в одном ряду.

                Виден ряд только тому, кто правит, и это ровно тот, кто
                отправляет: у чужого плана панели нет вовсе.
              */}
              {!onShelf && classId && !supervising && (
                <button
                  type="button"
                  className="secondary"
              ...
              {/*
                Тумблера видимости здесь больше нет — он переехал в шапку
                плашкой, той же, что в списках.

                Стоял он тут по верному доводу: состояний два, и оба надо
                назвать, а кнопка «Опубликовать» называет одно. Довод остался
                верным, ответ на него нашёлся лучше — плашка называет оба
                состояния и переключает сама. А панель действий тем временем
                держала **третью** форму того же вопроса: в шапке роль
                («черновик, видите только вы»), тут тумблер, в списках
                плашка. Один вопрос, три вида — и ни один не совпадал с тем,
                как то же самое показано там, откуда сюда приходят.
              */}
            ...
            {/*
              Строка утверждения: что с ним сейчас и **кто утверждает**.

              Второе тут новое, и без него первое отвечало только половине
              людей. «На утверждении у Петрова» видит тот, кто уже отправил;
              а до отправки экран молчал — кому именно уйдёт план, было не
              узнать, и кнопка «На утверждение» отправляла неизвестно кому.
              Хуже всего это у курса без методиста: нажимаешь и получаешь
              отказ, хотя сказать об этом можно было заранее.

              Поэтому строка стоит **всегда**, пока курс свой: слева
              состояние (если оно есть), справа — имя того, к кому план
              уходит, либо прямая правда, что назначать некого.

              Стоит она под рядом действий, а не в шапке: это ответ на
              «что вышло» у кнопки, которая теперь стоит здесь же.
            */}
            {!onShelf && classId && !supervising && (
              <p className="hint plan-approval-state">
                {baseline && (baseline.approved || baseline.request) && (
                  <span className={`approval ${baseline.request?.status ?? 'approved'}`}>
                    ...
                    {!baseline.request &&
                      ...
                      t(
                        ...
                        {
                          date: shortDate(
                            baseline.approved.approved_at.slice(0, 10),
                          ),
                ...
                {/* Кто утверждает. Пока запрос висит, имя уже названо строкой
                    состояния слева, и повторять его вторым разом незачем */}
                {baseline?.request?.status !== 'pending' && (
                  <span className="approver">
                    {baseline?.methodists?.length
                      ? t('plan.baseline.approver', {
                          names: baseline.methodists
                            .map((person) => person.name)
                            .join(', '),
                        })
                      : t('plan.baseline.noApprover')}
                  </span>
                )}

                {/* Свой запрос, свой же надзор: решать можно тут, но не
                    вместо плана. Ссылка ведёт в тот же экран надзора, каким
                    методист смотрит чужие курсы, — второго места для
                    «утвердить» и «вернуть» заводить незачем */}
                {selfReview && (
                  <span className="approval self">
