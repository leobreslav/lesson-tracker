 ...
 *
 * **Пустым законно и само условие.** Ячейка без текста — это вопрос, который
 * задали на доске или в книге, а здесь стоит ради балла. Окно отпускало без
 * него не всегда, и рядом со списком задач жила вторая кнопка — «пустая
 * ячейка», — существовавшая только затем, чтобы это окно обойти.
 *
    ...
    <Modal onClose={onClose} title={t(task ? 'works.task.edit' : 'works.task.add')}>
      <form onSubmit={submit}>
        ...
        <div className="actions">
          {/* Пустое условие — законное состояние, а не незаполненная форма.
              Оно и раньше было законным: ячейку без текста заводила соседняя
              кнопка «пустая ячейка», стоявшая в списке задач ровно затем,
              чтобы обойти это окно. Обход существовал потому, что окно не
              отпускало без условия, — то есть форма запрещала то, что модель
              разрешает, и приложение отвечало на это второй кнопкой.

              Теперь кнопка одна, а окно отпускает: условие на бумаге или на
              доске, а в ячейке — только балл. Дописать текст можно потом,
              руками или из банка. */}
